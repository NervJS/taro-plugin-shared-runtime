    ...
    const componentObj: Record<string, any> = {
        ...
        properties: {
            props: {
                ...
                observer(_newVal, oldVal) {
                    ...
                    oldVal && (this as any).component.forceUpdate()
                }
            }
        },
        created() {
            if (!(Current as any).root) {
        ...
        attached() {
            const runner = () => {
                ...
                setCurrent(this.compId);
                this.config = componentConfig;
                (Current as any).root!.mount!(Component, this.compId, () => this)
            }
            if ((Current as any).root) {
                runner()
            } else {
        ...
        detached() {
            (Current as any).root!.unmount!(this.compId)
        },
        pageLifetimes: {
            show(options) {
